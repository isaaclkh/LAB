...
class Dictionary:
    def __init__(self):
        self.Yes = ['yes', '네', '예', '응', '어', '있어', '좋아', '좋은', '좋다', '그래', '맞아', '알았어', '알겠어', '당연', '됐어','추천해','추천해줘', '좋았', '최고']
...
class NLP:
    ...
    def nlp_wrong(self, user_said, dic):
        answer = ''
        for i in range(len(dic.Wrong)):
            if dic.Wrong[i] in user_said:
                answer = 'WRONG'
        print(answer)
        return answer
