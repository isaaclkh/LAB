...
def Project_A(user_name):
    ...
    while True:
        ...
        text_to_speech(
            "내 얘기를 끝까지 들어줘서 고마워. 너를 위해서 선물을 준비했어.")
        user_said = input("답변 (남/여): ")
        oled_list.o_compliment()
        if user_said == '남':
            while True:
                time.sleep(1)
                text_to_speech("너는 귀여운 스티커를 좋아할 것 같으니까 이걸 선물로 줄게.")
                break
        else:
            while True:
                time.sleep(1)
                text_to_speech("너는 멋있는 스티커를 좋아할 것 같으니까 이걸 선물로 줄게.")
                break
        break
